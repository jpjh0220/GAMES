...
type MemoryEntry = {
  ...
  kind:'episode'|'strategy'|'relationship'|'place'|'lesson'|'observation'; text:string; importance:number;
...
export type AgentOutcome = {
  ...
  equipmentChanged:boolean;styleChanged:boolean;executionFailed:boolean;feedback?:string;
};
...
const itemPurpose=(name:string)=>{const n=String(name||'').trim();if(/^coins?$|^coin pouch$|^gp$/i.test(n))return 'keep';if(/^(bronze|iron|steel|mithril|adamant|rune) (axe|pickaxe)|tinderbox|fishing (net|rod)|fly fishing rod/i.test(n))return 'keep one operational copy; bank duplicates';if(/^bones?$/i.test(n)||/^(big )?bones$/i.test(n))return 'use/bury';if(/^raw /i.test(n)&&/(shrimp|anchov|fish|sardine|herring|trout|salmon)/i.test(n))return 'bank-or-cook';if(/^(cooked |burnt )?(shrimp|anchov|fish|sardine|herring|trout|salmon|lobster|bread|cake)/i.test(n))return 'eat/use';if(/^(ash|empty|junk|weed|rotten food)$/i.test(n))return 'discard';if(/(logs?|ore|bar|rune|coal|leather|herb|seed|quest|key)/i.test(n))return 'bank';return 'evaluate';};
...
export class SolAgentBrain {
  ...
  private strategistFailureStreak=0;
  private strategistRetryNotBefore=0;
  private strategy:Strategy|null=null;
  private liveObjective='';
  ...
  private feedbackMemory:{text:string;kind:'inventory_full'|'space_available'|'sdk_feedback'|'dialogue';tick:number;source:string;relatedAction:string|null}|null=null;
  ...
  private async loadRepoKnowledge(){
    ...
    this.gameplayCurriculum=[
      `CONSTITUTION ${GAMEPLAY_CONSTITUTION_VERSION}: ${GAMEPLAY_CONSTITUTION}`,
      `Repository lessons available: ${lessonNames.join(', ')}.`,
      `OPEN-ENDED PLAY GUIDANCE: Explore, inspect, interact, survive, experiment safely, and learn from verified outcomes. There is no permanent skill, level, fishing, banking, combat, or progression target. Choose temporary subgoals from observed opportunities and memory, then abandon or reframe them when evidence changes. Inventory warning is ${ECONOMY_POLICY.inventoryWarningSlots} slots remaining; repeated no-progress activities must switch method. A bank is persistent storage, not a universal dump; keep tools, currency, and usable food; use or bury Bones when appropriate; cook, bank, or sell raw food based on the self-selected subgoal; bank materials and quest items; discard only confirmed junk; every disposition must be justified by current evidence and verified by an observed state change.`
    ].join(' ');
  ...
  private observe(state:BotWorldState){
    ...
    for(const m of (state.gameMessages||[]) as any[]){
      const text=String(m.text||'').trim(),sender=String(m.sender||'').trim();if(!text)continue;
       ...
       const lower=text.toLowerCase();const incoming=!m.fromSelf&&!!sender;const kind=(/not enough inventory space|inventory is full|cannot carry|too full/.test(lower)?'inventory_full':/enough inventory space|has enough space|can hold that item/.test(lower)?'space_available':incoming?'dialogue':'sdk_feedback');
       this.feedbackMemory={text:text.slice(0,240),kind,tick:Number(m.tick)||0,source:sender||String(m.type||'world'),relatedAction:this.lastChoice?.fingerprint||null};
       this.addMemory({kind:'observation',text:`SDK feedback (${kind}): ${text}`,importance:kind==='inventory_full'?1.5:1.0,tags:['sdk-feedback',kind],tick:Number(m.tick)||0,state});
       console.log('AGENT_FEEDBACK_INTERPRETED',JSON.stringify(this.feedbackMemory));
      ...
      const person=norm(incoming?sender:this.opts.name);if(incoming&&person){
    ...
    const op:any=(state as any).opFeedback||{};const opText=String(op.message||op.lastMessage||op.error||op.reason||'').trim();
    if(opText){const opId=`op:${op.tick??(state as any).tick??''}:${opText}`;if(!this.seenMessages.has(opId)){this.seenMessages.add(opId);const lower=opText.toLowerCase();const kind=(/not enough inventory space|inventory is full|cannot carry|too full/.test(lower)?'inventory_full':/enough inventory space|has enough space|can hold that item/.test(lower)?'space_available':'sdk_feedback') as 'inventory_full'|'space_available'|'sdk_feedback';this.feedbackMemory={text:opText.slice(0,240),kind,tick:Number(op.tick||(state as any).tick)||0,source:'operation-feedback',relatedAction:this.lastChoice?.fingerprint||null};this.addMemory({kind:'observation',text:`SDK operation feedback (${kind}): ${opText}`,importance:kind==='inventory_full'?1.5:1.0,tags:['sdk-feedback',kind],tick:this.feedbackMemory.tick,state});console.log('AGENT_FEEDBACK_INTERPRETED',JSON.stringify(this.feedbackMemory));}}
    if(state.player){
  ...
  private antiLoopCandidates(candidates:AgentCandidate[]){
    ...
    for(const c of candidates){
      ...
      if(learned&&learned.n>=4&&(learned.failed||0)>=Math.ceil(learned.n/2))blocked.add(c.fingerprint);
  ...
  async decide(state:BotWorldState,candidates:AgentCandidate[],liveObjective?:string):Promise<AgentChoice>{
    ...
    if(!this.motorReady){
      if(Date.now()<this.motorRetryNotBefore)throw new Error(`AI motor retry delayed until ${new Date(this.motorRetryNotBefore).toISOString()}`);
      await this.refreshAvailability();
      if(!this.motorReady)throw new Error(`AI motor unavailable: ${this.motorModel}`);
    }
    // ---------------------------------------------------------------
    // PLANNER LAYER.
    //
    ...
    const shadowPrediction=this.lastShadowPrediction as AgentChoice|null;if(shadowPrediction){this.memory.lifetime.shadowPredictions=(this.memory.lifetime.shadowPredictions||0)+1;if(shadowPrediction.fingerprint===choice.fingerprint)this.memory.lifetime.shadowMatches=(this.memory.lifetime.shadowMatches||0)+1;}
    this.maybePromoteStudent();
    this.lastChoice=choice;this.markDirty();
    this.replay.push({tick:(state as any).tick||0,perception:`HP ${state.player?.hp}/${state.player?.maxHp}; ${state.nearbyPlayers?.length||0} agents; ${state.nearbyNpcs?.length||0} NPCs; ${state.nearbyLocs?.length||0} objects`,retrieved:this.currentGuidance,prediction:shadowPrediction?.fingerprint||null,decision:choice.reason,action:choice.fingerprint,outcome:null,lesson:null});
  ...
  private maybeRefreshStrategy(state:BotWorldState,candidates:AgentCandidate[]){
    ...
    // The motor owns the real-time control path. Do not launch the larger
    // strategist model until the motor has made a few stable choices, and back
    // off after a planning failure so it cannot monopolize the local runtime.
    const nowMs=Date.now();
    const initialPlanDue=this.strategy===null&&this.sessionMotorChoices>=3;
    const refreshDue=this.strategy!==null&&(this.sessionMotorChoices-this.lastStrategySessionChoice>=(this.runtimeConfig.strategyRefreshChoices||18)||stalled||repeatedCategory);
    const due=initialPlanDue||refreshDue;
    if(!due||this.strategistInFlight||nowMs<this.strategistRetryNotBefore)return;
    ...
    void this.askStrategist(state,candidates).then(strategy=>{
      ...
      this.lastStrategistError=null;this.strategistFailureStreak=0;this.strategistRetryNotBefore=0;this.lastStrategySessionChoice=this.sessionMotorChoices;this.strategy=strategy;this.memory.activePlan=strategy;this.memory.lifetime.strategistRefreshes=(this.memory.lifetime.strategistRefreshes||0)+1;this.markDirty();
    ...
    }).catch(err=>{this.lastStrategistError=String(err).slice(0,240);this.lastStrategySessionChoice=this.sessionMotorChoices;this.strategistFailureStreak++;const retryMs=Math.min(300000,30000*Math.pow(2,Math.min(3,this.strategistFailureStreak-1)));this.strategistRetryNotBefore=Date.now()+retryMs;this.memory.lifetime.strategistFailures=(this.memory.lifetime.strategistFailures||0)+1;console.warn('AI_STRATEGIST_ERROR',JSON.stringify({error:String(err),retryMs,failures:this.strategistFailureStreak}));}).finally(()=>{this.strategistInFlight=false;});
  }
  ...
  private async askMotor(state:BotWorldState,candidates:AgentCandidate[],contextKey:string):Promise<AgentChoice>{
    ...
    const motorObservation={
      plan:{objective:this.liveObjective||this.strategy?.objective||'Make measurable progress',active:this.strategy?.plan?.find(x=>x.status==='active')?.label||this.strategy?.focus||this.liveObjective||'Choose the next prerequisite'},
      ...
      feedback:this.feedbackMemory,
      guide:this.currentGuidance.slice(0,1).map(x=>x.slice(0,240)),
      // Keep the motor prompt within the fast local-model context budget. The
      // full legal set remains enforced server-side; this is its concise view.
      actions:allowed.slice(0,32).map(c=>[c.id,c.category,c.label.slice(0,72)])
    };
    try{
      const ask=async(payload:unknown,timeout:number)=>{const enriched=payload&&typeof payload==='object'?payload:{};const result=await chatJson({provider:this.runtimeConfig.provider||'ollama',model:this.motorModel,messages:[{role:'system',content:GAMEPLAY_CONSTITUTION+' You are the motor controller. Interpret the observed world and current temporary subgoal yourself, then select one offered action_id. The permanent mission is open-ended exploration, learning, survival, and play across the world. Prefer actions that expand Sol’s map, capabilities, resources, relationships, quest knowledge, equipment, or collection; local banking or combat is only worthwhile when it serves a temporary chosen subgoal. Do not force a skill, level, banking, fishing, combat, or progression activity unless the current evidence makes it purposeful. A dispatched action is only an attempt. Prefer an action that tests a hypothesis, learns a mechanic, discovers a place or entity, obtains a useful capability, interacts meaningfully, or safely advances the current subgoal. If inventory is full or near full, resolve capacity before risky activity. After combat, verify and resolve ground loot before leaving. Avoid wait when another purposeful action exists. The feedback field is authoritative SDK evidence, not optional chat: explicitly interpret its exact text, reconcile it with inventory and world state, and do not repeat the related action unless new evidence changes the conclusion. If feedback says an item cannot be carried or inventory is full, choose a capacity-changing or alternative action; if feedback says space is available but the pickup did not change inventory, infer another prerequisite and investigate it. speech is empty unless selecting a say action. Do not infer that an action is good because it was previously rewarded.'},{role:'user',content:JSON.stringify(enriched)}],schema,temperature:this.runtimeConfig.motorTemperature||.12,contextTokens:this.runtimeConfig.motorContextTokens||2048,outputTokens:this.runtimeConfig.motorOutputTokens||72,ollamaUrl:this.ollamaUrl,geminiApiKey:process.env.GEMINI_API_KEY,timeoutMs:timeout});return parseModelJson(result.content);};
      let j:any;try{j=await ask(motorObservation,12000)}catch(first){j=await ask({plan:motorObservation.plan,actions:motorObservation.actions,instruction:'Choose one listed action now. Prefer movement, interaction, or resource acquisition over waiting.'},7500)}
      const c=allowed.find(x=>x.id===j.action_id);if(!c)throw new Error(`invalid motor action ${j.action_id}`);
      const rawWhy=String(j.why||'').trim();const labelTerms=[...toks(c.label)].filter(x=>!/^\d+$/.test(x)&&x!=='distance');const whyTerms=toks(rawWhy);const reasonMatches=labelTerms.length===0||labelTerms.some(x=>whyTerms.has(x));const why=(c.action?.type==='pickupItem'&&!reasonMatches?`Selected ${c.label}; verify the pickup result and any SDK feedback before choosing what to do next.`:(rawWhy||`Selected ${c.label}`)).slice(0,180),goal=String(this.liveObjective||this.strategy?.focus||`Progress through ${c.category}`).slice(0,180),followUp:string[]=[],planNote='Immediate action chosen by the motor model.',speech=String(j.speech||'').slice(0,80);
      this.motorFailures=0;this.motorRetryNotBefore=0;
      ...
      this.motorFailures++;this.teacherConsecutiveFailures++;const errorText=String(err);this.lastTeacherError=errorText.slice(0,240);
      const quota=/\b429\b|quota|rate limit/i.test(errorText);
      const retryMs=quota?300000:Math.min(90000,5000*Math.pow(2,Math.min(4,this.motorFailures-1)));
      this.motorRetryNotBefore=Date.now()+retryMs;
      if(this.teacherConsecutiveFailures>=3)this.motorReady=false;
      console.warn('AI_MOTOR_BACKOFF',JSON.stringify({error:this.lastTeacherError,retryMs,failures:this.motorFailures,quota}));
      throw err;
    }
  }
  ...
  maybeFinishExperience(state:BotWorldState,tick:number):AgentOutcome|null{
    ...
    const xpGain=Math.max(0,after.totalXp-exp.before.totalXp);const levelGain=Math.max(0,after.totalLevels-exp.before.totalLevels);const hpDelta=after.hp-exp.before.hp;const moved=after.x!==exp.before.x||after.z!==exp.before.z||after.level!==exp.before.level;const died=after.lifeId!==exp.before.lifeId||(exp.before.hp>0&&after.hp<=0);const sdkReportedFailure=exp.candidate.action?.executionResult?.success===false;let executionFailed=sdkReportedFailure;let rejected=after.opRejectedCount>exp.before.opRejectedCount||executionFailed;
    ...
    const inventoryChanged=after.inventorySig!==exp.before.inventorySig;const equipmentChanged=after.equipmentSig!==exp.before.equipmentSig;const opFeedbackText=String((state as any).opFeedback?.message||(state as any).opFeedback?.lastMessage||(state as any).opFeedback?.error||(state as any).opFeedback?.reason||'').trim();const actionFeedbackText=String(exp.candidate.action?.executionResult?.message||'').trim();const verifiedWorldChange=moved||xpGain>0||levelGain>0||kills>0||damageDealt>0||inventoryChanged||equipmentChanged; if(verifiedWorldChange){executionFailed=false;rejected=false;} const styleChanged=after.combatStyleSig!==exp.before.combatStyleSig;const feedbackText=[...(state.gameMessages||[]).filter((m:any)=>Number(m.tick||0)>=exp.startTick).map((m:any)=>String(m.text||'').trim()),opFeedbackText,actionFeedbackText].filter(Boolean).slice(-3).join(' | ').slice(0,420);if(actionFeedbackText){const lower=actionFeedbackText.toLowerCase();const kind=(/not enough inventory space|inventory is full|cannot carry|too full/.test(lower)?'inventory_full':/enough inventory space|has enough space|can hold that item/.test(lower)?'space_available':'sdk_feedback') as 'inventory_full'|'space_available'|'sdk_feedback';this.feedbackMemory={text:actionFeedbackText.slice(0,240),kind,tick,source:'action-execution-result',relatedAction:exp.candidate.fingerprint};if((kind==='space_available'||kind==='inventory_full')&&!inventoryChanged){this.blockedFingerprints=[...new Set([...this.blockedFingerprints,exp.candidate.fingerprint])].slice(-80);this.addMemory({kind:'lesson',text:`Do not immediately retry ${exp.candidate.label}: SDK feedback was “${actionFeedbackText}” but inventory did not change. Investigate a different prerequisite or choose another action.`,importance:1.8,tags:['sdk-feedback','pickup-constraint',kind],tick,state});}this.addMemory({kind:'observation',text:`Action feedback (${kind}): ${actionFeedbackText}`,importance:kind==='inventory_full'?1.6:1.2,tags:['sdk-feedback',kind,exp.candidate.category],tick,state});console.log('AGENT_FEEDBACK_INTERPRETED',JSON.stringify(this.feedbackMemory));}const executionMessage=executionFailed?String(exp.candidate.action?.executionResult?.message||'').slice(0,420):'';const discovers=/^(explore|navigation-skill|world)$/.test(exp.candidate.category);const configures=/^(inventory|combat-style)$/.test(exp.candidate.category);const progressionAction=exp.candidate.category==='combat'||exp.candidate.action?.type==='worldSkill'||/(fish|fishing|mine|mining|woodcut|chop|smith|craft|train|attack|kill)/i.test(String(exp.candidate.label||''));const creditedXpGain=progressionAction?xpGain:0;const creditedLevelGain=progressionAction?levelGain:0;
    let reward=Math.min(4,creditedXpGain*.02)+creditedLevelGain*3+damageDealt*.15+kills*3-damageTaken*.25;if(hpDelta<0)reward+=hpDelta*.15;if(moved)reward+=discovers?.35:.08;if(discovers)reward+=Math.min(1.2,newNpc.length*.2+newLocs.length*.1);if(exp.candidate.category==='social'||exp.candidate.category==='say')reward+=Math.min(.3,newPlayers.length*.15);reward+=clamp(coinGain*.02,-.5,1.5)+clamp(inventoryGain*.08,-.4,.5);if(configures&&equipmentChanged)reward+=.12;if(configures&&styleChanged)reward+=.08;if(rejected)reward-=1.25;if(died)reward-=10;
    ...
    if(!moved&&creditedXpGain===0&&kills===0&&damageDealt===0&&coinGain===0&&inventoryGain===0){
    ...
    const noProgress=!moved&&creditedXpGain===0&&kills===0&&damageDealt===0&&coinGain===0&&inventoryGain===0&&!(configures&&(equipmentChanged||styleChanged))&&!unlocked.length;if(noProgress)reward-=exp.candidate.category==='wait'?.15:.6;reward=Number(clamp(reward,-10,10).toFixed(3));
    const newThings=[...newPlayers.map(x=>`agent:${x}`),...newNpc.slice(0,4).map(x=>`npc:${x}`),...newLocs.slice(0,4).map(x=>`loc:${x}`)];
    const parts=[creditedXpGain?`+${creditedXpGain} XP`:'',creditedLevelGain?`+${creditedLevelGain} level(s)`:'',damageDealt?`${damageDealt} damage dealt`:'',damageTaken?`${damageTaken} damage taken`:'',kills?`${kills} kill(s)`:'',moved?`moved ${exp.before.x},${exp.before.z} → ${after.x},${after.z}`:'',inventoryChanged?'inventory changed':'',equipmentChanged?'equipment changed':'',styleChanged?'combat style changed':'',coinGain?`${coinGain>0?'+':''}${coinGain} coins`:'',rejected?'execution rejected or failed':'',executionMessage?`detail: ${executionMessage}`:'',died?'died/respawned':'',unlocked.length?`unlocked: ${unlocked.join(', ')}`:'',discovers&&newThings.length?`new: ${newThings.slice(0,6).join(', ')}`:''].filter(Boolean);
    const outcome:AgentOutcome={tick,reward,summary:parts.length?parts.join('; '):'No measurable change.',choice:exp.choice,candidateLabel:exp.candidate.label,category:exp.candidate.category,actionType:String(exp.candidate.action?.type||'unknown'),xpGain:creditedXpGain,hpDelta,moved,kills,damageDealt,damageTaken,newThings,rejected,inventoryChanged,inventoryGain,coinGain,noProgress, equipmentChanged,styleChanged,executionFailed,feedback:feedbackText||undefined};
  ...
  publicState(){
    ...
    return{architecture:'model-sovereign-open-ended-play-with-execution-validation',provider:this.runtimeConfig.provider||'ollama',geminiConfigured:!!process.env.GEMINI_API_KEY,externalDirective:this.externalDirective,teacherModel:this.motorModel,motorModel:this.motorModel,strategistModel:this.strategistModel,teacherOnline:this.motorReady,motorOnline:this.motorReady,strategistOnline:this.strategistReady,motorFailures:this.motorFailures,motorRetryAt:this.motorRetryNotBefore?new Date(this.motorRetryNotBefore).toISOString():null,teacherConsecutiveFailures:this.teacherConsecutiveFailures,lastTeacherHealthyAt:this.lastTeacherHealthyAt?new Date(this.lastTeacherHealthyAt).toISOString():null,lastTeacherError:this.lastTeacherError,studentMode:'shadow-only',currentController:this.motorReady?'teacher-model':'deterministic-recovery',autonomy:{modelControlsGameplay:true,heuristicActionRanking:false,executionValidation:true,emergencyOverride:false,lastProposal:this.lastAutonomousProposal},strategy:this.strategy||this.memory.activePlan,planTree:(this.strategy||this.memory.activePlan)?.plan||[],strategistInFlight:this.strategistInFlight,lastStrategistError:this.lastStrategistError,strategistFailureStreak:this.strategistFailureStreak,strategistRetryAt:this.strategistRetryNotBefore?new Date(this.strategistRetryNotBefore).toISOString():null,sessionMotorChoices:this.sessionMotorChoices,lastChoice:this.lastChoice,lastOutcome:this.lastOutcome,blockedFingerprints:this.blockedFingerprints,shadowStudentPrediction:this.lastShadowPrediction,retrievedRepoKnowledge:this.currentGuidance,shadowAgreementRate:predictions?Number((matches/predictions).toFixed(3)):null,learnedContexts:contexts,learnedActions,memoryCount:this.memory.memories.length,relationshipCount:Object.keys(this.memory.relationships).length,placeCount:Object.keys(this.memory.places).length,repoKnowledgeSegments:this.repoKnowledge.length,repoKnowledgeSources:new Set(this.repoKnowledge.map(x=>x.source)).size,relationships,worldDiscoveries:this.memory.discoveries.slice(-30).reverse(),learnedSequences:sequences,mindReplay:this.replay.slice(-30).reverse(),lifetime:this.memory.lifetime,recentMemories:this.memory.memories.slice(-12),recentOutcomes:this.recentOutcomes.slice(-12)};
  }
}
