...
export class SolAgentBrain {
  ...
  private async askStrategist(state:BotWorldState,candidates:AgentCandidate[]):Promise<Strategy>{
    ...
    const observation={directive:this.externalDirective,position:[p.worldX,p.worldZ,p.level],hp:[p.hp,p.maxHp],combatLevel:p.combatLevel,skills:skillLevels,coins,inventory:(state.inventory||[]).slice(0,16).map(i=>[i.name,i.count]),itemPolicy:(state.inventory||[]).slice(0,16).map(i=>[i.name,itemPurpose(i.name)]),equipment:(state.equipment||[]).map(i=>i.name),failed:negatives,actions:executable,nearby:(state.nearbyLocs||[]).slice(0,8).map((loc:any)=>loc.name),curriculum:this.gameplayCurriculum,guide:guidance[0]||''};
