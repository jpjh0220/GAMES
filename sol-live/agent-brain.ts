...
export class SolAgentBrain {
  ...
  private async loadRepoKnowledge(){
    ...
    this.gameplayCurriculum=[
      ...
      `ECONOMY LIMITS: inventory warning at ${ECONOMY_POLICY.inventoryWarningSlots} slots remaining; at most ${ECONOMY_POLICY.maxFishingTripsWithoutProgress} fishing trips without progress; at most ${ECONOMY_POLICY.maxRepeatedCombatTarget} repeated combat attempts on one target; switch after ${ECONOMY_POLICY.maxActionsWithoutMilestone} actions without a milestone. ITEM PURPOSE: a bank is persistent storage, not a universal dump; keep tools, currency, and usable food; use or bury Bones when appropriate; cook, bank, or sell raw food based on the next goal; bank materials and quest items; discard only confirmed junk; every disposition must be justified by a future objective and verified by an observed state change.`
    ].join(' ');
