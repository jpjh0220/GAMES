...
export class ProgressionDirector{
  ...
  plan(state:BotWorldState,candidates:AgentCandidate[],tick:number):ProgressionPlan{
    ...
    if(free<=3)return plan('resolve-capacity','Resolve inventory capacity before training','Space is a prerequisite for every productive loop. Prefer depositing at a bank; use a merchant only when no bank route is visible, then verify free slots increased.','At least 8 free inventory slots.',capacityRoute,0,true);
