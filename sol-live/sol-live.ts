...
const publicSnapshot=()=>({
  ...
  combatStyle:snapshot.combatStyle||null,nearbyNpcs:snapshot.nearbyNpcs||[],nearbyPlayers:snapshot.nearbyPlayers||[],groundItems:snapshot.groundItems||[],nearbyLocs:snapshot.nearbyLocs||[],opFeedback:snapshot.opFeedback||{opRejectedCount:0},sessionEnd:snapshot.sessionEnd||null,inventory:snapshot.inventory||[],equipment:snapshot.equipment||[],combatEvents:(snapshot.combatEvents||[]).slice(-80),gameMessages:[],outgoingChat:[],recentDialogs:[],actions:(snapshot.actions||[]).slice(-40),cognitionLog:(snapshot.cognitionLog||[]).slice(-80).map(({state,...event}:any)=>event),movementTrail:(snapshot.movementTrail||[]).slice(-200),lessons:(snapshot.lessons||[]).slice(-40),
...
const startSessionWithLoginRetry=async()=>{
  ...
  for(let attempt=0;attempt<5;attempt++){
    try{
      ...
      return await Promise.race([attempt,Bun.sleep(35000).then(()=>{const timeout:any=new Error('SDK login attempt timed out');timeout.code='SDK_LOGIN_TIMEOUT';throw timeout;})]);
    }catch(err){
      lastError=err;
...
const WORLD_ATLAS=[
  {id:'lumbridge',name:'Lumbridge',x:3222,z:3218,waypoints:[[3130,3260],[3160,3240],[3180,3225],[3200,3224],[3222,3218]],tags:['town','quest','social','training','bank']},
  {id:'varrock',name:'Varrock',x:3210,z:3424,waypoints:[[3130,3320],[3150,3350],[3170,3370],[3190,3400],[3210,3424]],tags:['city','quest','market','combat','social']},
  {id:'falador',name:'Falador',x:2965,z:3380,waypoints:[[3040,3320],[3020,3340],[3000,3360],[2980,3370],[2965,3380]],tags:['city','quest','mining','combat','social']},
  {id:'al-kharid',name:'Al Kharid',x:3293,z:3179,waypoints:[[3180,3220],[3210,3210],[3240,3195],[3270,3185],[3293,3179]],tags:['desert-edge','trade','combat','quest']},
...
const buildCandidates=(state:BotWorldState):AgentCandidate[]=>{
  ...
  if(state.shop?.isOpen){
    ...
    if(economyResolution.noProgress<2)for(const item of (state.shop.playerItems||[]).filter((i:any)=>{const price=Number(i.sellPrice||0);const disposition=itemDisposition(i,state);return !isCurrency(i.name)&&price>0&&(disposition==='sell'||disposition==='discard'||/(junk|ash|weed|bones|empty)/i.test(String(i.name||'')));}).slice(0,10)) add({label:`Sell 1 ${item.name} for about ${item.sellPrice}`,category:'shop',fingerprint:`shop:sell:${norm(item.name)}`,settleTicks:5,action:{type:'shopSell',slot:item.slot,amount:1,reason:pressure?'Sell surplus to create inventory capacity.':'Sell a policy-approved surplus item for coins.'},tags:['economy','sell','policy-approved',item.name]});
